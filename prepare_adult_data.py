...
"""
	...
	Data processing from Zafar et. al 2017
...
def load_adult_data(load_data_size=None):
	...
	def convert_attrs_to_ints(d): # discretize the string attributes
		...
		for attr_name, attr_vals in d.items():
			...
			uniq_vals = sorted(list(set(attr_vals))) # get unique values would ensure that female/black comes first hence assigned 0
			#print("attr_name: " , attr_name)
			# compute integer codes for the unique values
			val_dict = {}
			for i in range(0,len(uniq_vals)):
				val_dict[uniq_vals[i]] = i
				#print( "uniq vals: ", uniq_vals[i], " val ", val_dict[uniq_vals[i]])
	...
	if scaling :
		...
		for attr_name in attrs_for_classification:
			attr_vals = attrs_to_vals[attr_name]
			X_raw.append(attr_vals) # would also like save raw features 
			if attr_name in int_attrs or attr_name == "native_country": # the way we encoded native country, its binary now so no need to apply one hot encoding on it
				...
				#print( " Adult data dimension " , attr_vals.shape)
				
				attr_vals = lb.fit_transform(attr_vals)	# returns N x D where D are the dimensions of converted labels		
				for inner_col in attr_vals.T:	# transpose is putting features in rows and loops picks a row and adds it as a column in X 			
					X.append(inner_col) 
			
			
			#print( " Adult data dimension through lb " , attr_vals.shape)
	## scaling int_attributes 
	...
	# uncommment lines below ************	

	perm = np.arange(0,X.shape[0])
	#rand_state = np.random.RandomState(1125253344)
	#rand_state.shuffle(perm)
